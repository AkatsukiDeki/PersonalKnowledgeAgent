...
5. Если в тексте упомянут временной интервал или точная дата события, обязательно заполни поля `valid_from` и `valid_to` в формате `YYYY-MM-DD` (или `YYYY-MM` / `YYYY`, если точный день неизвестен). Если дата относительная, рассчитай ее исходя из контекста документа. Сохрани оригинальную фразу в `temporal_context`.
"""
