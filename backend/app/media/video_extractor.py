...
class VideoSlideExtractor:
    def __init__(self, ssim_threshold: float = 0.88, scene_threshold: float = 0.4, max_slides: int = 60):
    ...
    def _extract_keyframes_with_timestamps(
        ...
        ffmpeg_cmd = [
            ...
            "-y",
            "-i", video_path,
            "-vf", f"select='gt(scene,{self.scene_threshold})+gte(t-prev_selected_t,25)',format=yuvj420p,showinfo",
            "-fps_mode", "vfr",
            "-q:v", "3",
            output_pattern
        ]

        logger.info(f"[VideoExtractor] Running optimized FFmpeg scene extraction for {video_path}...")
        ...
        for i, frame_path in enumerate(frame_files):
            ts = timestamps[i] if i < len(timestamps) else float(i * 25.0)
    ...
    def _calculate_ssim(self, img_path_a: Path, img_path_b: Path) -> float:
        ...
        img_a = cv2.resize(img_a, (320, 180))
        img_b = cv2.resize(img_b, (320, 180))
    ...
    def extract_unique_slides(
        ...
        for ts, frame_path in raw_keyframes:
            ...
            is_duplicate = False
            for prev_frame in recent_saved_frames[-3:]:
                if self._calculate_ssim(prev_frame, frame_path) >= self.ssim_threshold:
                    is_duplicate = True
                    try:
                        os.remove(frame_path)
                    except OSError:
            ...
            if not is_duplicate:
                ...
                recent_saved_frames.append(frame_path)
