...
### Август 2026 (M3.1 / M3.2 - Media Intelligence)
- Реализована трехуровневая фильтрация аудио: детектирование обычных аудиофайлов/подкастов и личных голосовых заметок.
- Исправлена проблема "галлюцинаций" модели Whisper (зацикливание на тишине/музыке): отключен `condition_on_previous_text`, включен `vad_filter`, добавлены регулярные выражения для очистки музыкальных тегов (типа `♪` и `[Музыка]`).
- Устранен polling-storm на фронтенде: отключены лишние интервалы проверок в компонентах `ConversationSidebar` и `Sidebar`.
- Исправлена ошибка `409 Conflict` (и изменена на `404`) при попытке транскрибировать утерянные/legacy медиафайлы. Пути к файлам теперь резолвятся относительно `/app` для корректной работы в Docker-контейнере.
- Решена серия проблем БД (ошибка `500 Internal Server Error`): 
  - Исправлена опечатка (`metadata_info` -> `meta_info`) при формировании `SourceResponse`.
  - В SQLAlchemy-модели `Source` и `Chunk` добавлено зеркалирующее поле `metadata_info`, чтобы удовлетворить констрейнт `NOT NULL` старой схемы в PostgreSQL при вставке новых записей, избегая `NotNullViolationError`.
- **Где мы остановились:** Бэкенд успешно пересобран и работает в Docker Compose (VPS). Конвейер загрузки и транскрибации (Media Ingestion) полностью функционален, вставка в БД (чанков и эмбеддингов) работает корректно, UI очищен от лишних запросов. Готовы к тестированию пайплайна транскрибации через интерфейс и развитию UI (M3.2).
